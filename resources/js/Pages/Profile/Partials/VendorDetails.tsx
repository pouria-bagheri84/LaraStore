...
export default function VendorDetails(
    ...
    const becomeVendor : FormEventHandler = (ev)=>{
        ...
        post(route('vendor.store'), {
            ...
            onSuccess: ()=>{
                closeModal()
                setSuccessMessage('You can now create and publish your products.')
            },
            onError: (err)=>{
            ...
            <header>
                <h2 className={'flex justify-between mb-8 text-lg font-medium text-gray-900 dark:text-gray-100'}>
                    ...
                    {user.vendor?.status === 'pending' && (
                        <span className={'badge badge-warning badge-lg text-gray-100'}>{user.vendor.status_label}</span>
                    )}
                    {user.vendor?.status === 'rejected' && (
                        <span className={'badge badge-error badge-lg text-gray-100'}>{user.vendor.status_label}</span>
                    )}
                    {user.vendor?.status === 'approved' && (
                        <span className={'badge badge-success badge-lg text-gray-100'}>{user.vendor.status_label}</span>
                    )}
                </h2>
            </header>
            ...
            <Modal show={showBecomeVendorConfirmation} onClose={closeModal}>
                <form className={'p-8'} onSubmit={becomeVendor}>
                    ...
                    <div className={'mt-6 flex justify-end gap-4'}>
                        ...
                        <PrimaryButton className={'ms-3 btn btn-primary'} disabled={processing}>
                            Confirm
                        </PrimaryButton>
                    </div>
                </form>
